				...
				<fieldset id="simple-local-avatar-ratings" <?php disabled( empty( $profileuser->simple_local_avatar ) ); ?>>
					...
					<?php
						...
						foreach ( $this->avatar_ratings as $key => $rating ) :
							echo "\n\t<div><label><input type='radio' name='simple_local_avatar_rating' value='" . esc_attr( $key ) . "' " . checked( $profileuser->simple_local_avatar_rating, $key, false ) . "/> $rating</label></div>";
						endforeach;
					?>
