...
class ZillaLikes {
	...
	function setting_show_on()
	{
		...
		if( !isset($options['add_to_images']) ) $options['add_to_images'] = '0';
		if( !isset($options['add_to_videos']) ) $options['add_to_videos'] = '0';
		...
		<input type="hidden" name="zilla_likes_settings[add_to_images]" value="0" />
		<label><input type="checkbox" name="zilla_likes_settings[add_to_images]" value="1"'. (($options['add_to_images']) ? ' checked="checked"' : '') .' />
		'. __('Images', 'zilla') .'</label><br />		
		<input type="hidden" name="zilla_likes_settings[add_to_videos]" value="0" />
		<label><input type="checkbox" name="zilla_likes_settings[add_to_videos]" value="1"'. (($options['add_to_videos']) ? ' checked="checked"' : '') .' />
		'. __('Videos', 'zilla') .'</label><br />		
	...
	{		
		...
		if( 'post' == get_post_type() && $options['add_to_posts']) $content .= $this->do_likes();
		if( 'images' == get_post_type() && $options['add_to_images']) $content .= $this->do_likes();
		if( 'videos' == get_post_type() && $options['add_to_videos']) $content .= $this->do_likes();
